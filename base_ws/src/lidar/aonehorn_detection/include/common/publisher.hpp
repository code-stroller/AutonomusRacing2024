...
void publish2DVector(const ros::Publisher & pub, const std_msgs::Header & header, const std::vector<std::vector<double>>&vec2d) {
    std_msgs::Float32MultiArray msg;

    // layout 설정: 2차원 배열 (dim[0]은 행(row), dim[1]은 열(column))
    msg.layout.dim.resize(2);
    msg.layout.dim[0].label = "rows";
    msg.layout.dim[0].size = vec2d.size();  // 행의 수
    msg.layout.dim[0].stride = vec2d.size() * (vec2d.empty() ? 0 : vec2d[0].size());
    msg.layout.dim[1].label = "columns";
    msg.layout.dim[1].size = vec2d.empty() ? 0 : vec2d[0].size();  // 열의 수
    msg.layout.dim[1].stride = vec2d.empty() ? 0 : vec2d[0].size();

    // 2차원 벡터 데이터를 1차원 배열로 변환하여 메시지에 추가
...
static void publishMarkerarray(
    const ros::Publisher& publisher,
    const std_msgs::Header& header,
    const std_msgs::ColorRGBA& color,
    const std::vector<std::vector<double>>& coooneees,
    const Eigen::Matrix4d& trans = Eigen::Matrix4d::Zero()) {
    // clear all markers before
    visualization_msgs::MarkerArray empty_markers;
    visualization_msgs::Marker clear_marker;
    clear_marker.header = header;
    clear_marker.ns = "markerarr_cone";
    clear_marker.id = 0;
    clear_marker.action = clear_marker.DELETEALL;
    clear_marker.lifetime = ros::Duration();
    empty_markers.markers.push_back(clear_marker);
    ...
    for (size_t i = 0u; i < coooneees.size(); ++i) {
        visualization_msgs::Marker marker;

        // 마커 ID 설정 (각 마커는 고유한 ID를 가져야 합니다)
        marker.id = i;
        ...
        // 마커 액션 설정 (ADD: 새로 추가하거나 갱신)
        marker.action = visualization_msgs::Marker::ADD;

        // 마커의 위치 설정
        marker.pose.position.x = coooneees[i][0];
        marker.pose.position.y = coooneees[i][1];
        ...
        // markers 벡터에 마커 추가
        cone_markers.markers.push_back(marker);
    }

    publisher.publish(cone_markers);
}
//jaeseung
...
static void publishTrackingObjects(
    ...
    if (!tracking_objects.empty()) {
        ...
        for (size_t i = 0u; i < tracking_objects.size(); ++i) {
            ...
            msg_tracking_object.height  = obj->height;
