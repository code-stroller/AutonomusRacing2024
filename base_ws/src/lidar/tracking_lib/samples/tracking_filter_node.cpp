 ...
 */
 ...
 /*
    2024-반사율 필터링통하여 배달미션중 표지판 - 지면 노이즈 제거관련 코드 추가 -박정웅, 황성하
 */
...
void OnSegmentClouds(
    ...
    for (size_t i = 0; i < segment_clouds.size(); ++i) {
        vec2d.push_back(std::vector<double>{
            objects[i]->ground_center[0],
                objects[i]->ground_center[1],
                objects[i]->length,
                objects[i]->width,
                objects[i]->height,
                static_cast<double>(segment_clouds[i]->size()),  // Dereference and cast to double
                objects[i]->yaw_rad
        });
    }
    //ROS_INFO_STREAM("vec2d size: " << vec2d.size());
    ...
    autosense::common::publishTrackingObjects(tracking_output_objects_pub_,
                                              header, tracking_objects_world, car_vel);
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "tracking_filtering_node");
    ...
    std::string pub_output_objects_filtered_topic;

    private_nh.getParam(
        param_ns_prefix_ + "/pub_output_objects_filtered_topic",
        pub_output_objects_filtered_topic);

    std::string pub_objects_filtered_topic;

    private_nh.getParam(
        param_ns_prefix_ + "/pub_objects_filtered_topic",
        pub_objects_filtered_topic);
