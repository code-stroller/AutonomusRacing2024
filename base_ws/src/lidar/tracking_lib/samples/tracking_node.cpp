...
ros::Publisher cloudconepublisher;
ros::Publisher conepublisher;
...
void OnSegmentClouds(
    ...
    for (size_t i = 0; i < tracking_objects_world.size(); ++i) {
        vec2d.push_back(std::vector<double>{
            tracking_objects_world[i]->ground_center[0],
                tracking_objects_world[i]->ground_center[1],
                tracking_objects_world[i]->length,
                tracking_objects_world[i]->width,
                tracking_objects_world[i]->height,
                tracking_objects_world[i]->height,
                tracking_objects_world[i]->yaw_rad
        });
    }

    autosense::common::publishMarkerarray(cloudconepublisher, header,
        autosense::common::CYAN.rgbA,
        vec2d);

    autosense::common::publish2DVector(conepublisher, header, vec2d);
...
int main(int argc, char **argv) {
    ...
    pcs_segmented_sub_ = nh.subscribe<autosense_msgs::PointCloud2Array>(
        sub_pcs_segmented_topic, 1, OnSegmentClouds);
    ...
    cloudconepublisher =
        private_nh.advertise<visualization_msgs::MarkerArray>(
            "cooonnnne", 1);
    conepublisher = private_nh.advertise<std_msgs::Float32MultiArray>("cone", 1);
