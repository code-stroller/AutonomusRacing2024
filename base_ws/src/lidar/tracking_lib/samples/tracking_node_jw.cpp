...
void OnSegmentClouds(
    ...
    for (size_t i = 0; i < segment_clouds.size(); ++i) {
        ...
        if (objects[i]->length < 0.5 && objects[i]->width < 0.5) {
            /*std::tuple<bool, float, float> ajc;
            ajc = detectCone(segment_clouds[i]);
            
            if (std::get<0>(ajc)) {
                
                vec2d.push_back(std::vector<double>{objects[i]->ground_center[0], objects[i]->ground_center[1]});
