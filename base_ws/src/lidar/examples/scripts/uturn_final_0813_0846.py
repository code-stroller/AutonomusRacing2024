        ...
        for left_index in left:
            for right_index in right:
                if self.euclidean_distance(left_index,right_index)>self.min_distance_threshold:
                    return False   
        return True
    def transform_to_gps(sensor_obj_coords, car_coords, car_yaw):
        """
        센서 좌표계에서 GPS 좌표계로 물체 좌표를 변환합니다.
        :param sensor_obj_coords: (x, y) 센서 좌표계에서의 물체 좌표
        :param car_coords: (x_car, y_car) GPS 좌표계에서의 차량 좌표
        :param car_yaw: 차량의 방향(야우, radian 단위)
        :return: (x_gps, y_gps) GPS 좌표계에서의 물체 좌표
        """
        # 센서 좌표계를 차량 좌표계로 변환
        x_car_obj = sensor_obj_coords[0]
        y_car_obj = sensor_obj_coords[1]

        # 차량 좌표계를 GPS 좌표계로 변환
        x_gps_obj = math.cos(car_yaw) * x_car_obj - math.sin(car_yaw) * y_car_obj + car_coords[0]
        y_gps_obj = math.sin(car_yaw) * x_car_obj + math.cos(car_yaw) * y_car_obj + car_coords[1]
    return x_gps_obj, y_gps_obj
        ...
        for i, obj in enumerate(obj):
            ...
            if obj.height ==1.2345678806304932:
                ...
                for obs in self.obstacle_list:
                    obs_tf = transform_to_gps([bbox_center_x, bbox_center_y],(self.x,self.y),self.vehicle_yaw)
                    

                    dista = self.euclidean_distance(obs[:2],obs_tf)
                    if dista<0.2:
                        find_obs = True
                        obs[2] +=1
                        if obs[2] ==3:
                        ...
                        break
                if !find_obs:
                    self.obstacle_list.append([obs_tf[0],obs_tf[1],0])
