...
class GPS2UTM:
    ...
    def should_add_cone(self, cones_list, new_cone, min_distance):
        ...
        N1 = self.euclidean_distance(last_cone, new_cone) < min_distance
        if len(cones_list)<2:
            return N1
        else:
            v1angle = [cones_list[-1][0] - cones_list[-2][0],cones_list[-1][1] - cones_list[-2][1]]
            v2angle = [new_cone[0] - cones_list[-1][0],new_cone[1] - cones_list[-1][1]]

            v1angle = np.arccos(v1angle[0]/(v1angle[0]**2+v1angle[1]**2)**0.5)*180/math.pi
            v2angle = np.arccos(v2angle[0]/(v2angle[0]**2+v2angle[1]**2)**0.5)*180/math.pi
            return N1 and v2angle < v1angle
    ...
    def find_bef_obj(self, bef,obj):
        for i,bef_obj in enumerate(bef):
            if self.euclidean_distance((bef_obj[0],bef_obj[1]),obj[:2])<0.8:
                bef_obj[2] +=3
                bef_obj[2] = min(bef_obj[2],18)
                bef_obj[0]=obj[0]
                bef_obj[1]=obj[1]
                return i, True
        return -1,False
    ...
    def lidar_callback(self, _data):
        ...
        for i, obj in enumerate(obj):
            ...
            #print(obj.height)
            if obj.height ==1.2345678806304932 and 0<bbox_width<0.3 and 0<bbox_height<0.3:
                obj_collector.append([bbox_center_x, bbox_center_y,bbox_width, bbox_height])
        
        drift = []  
        
        if self.before_obstacle != []:
            for before_obj in self.before_obstacle:
                before_obj[2]-=1
                for after_obj in obj_collector:
                    if self.euclidean_distance(before_obj,after_obj) <0.1:
                        drift.append([after_obj[0]-before_obj[0],after_obj[1]-before_obj[1]])
            if drift:
                drift = np.average(np.array(drift), axis=0)
            else:
                drift = np.array([0, 0]) 
            afterX_befO = [k for k in range(len(self.before_obstacle))]
            print(afterX_befO)
            for after_obj in obj_collector:
                ind , found = self.find_bef_obj(self.before_obstacle,after_obj)
                if not found:
                    self.before_obstacle.append([after_obj[0],after_obj[1],3])
                    afterX_befO.remove(ind)
            for afterX_befO_index in afterX_befO:
                self.before_obstacle[afterX_befO_index][0]+=drift[0]
                self.before_obstacle[afterX_befO_index][1]+=drift[1]
        else:
            for init_obj_collected in obj_collector:
                self.before_obstacle.append([init_obj_collected[0],init_obj_collected[1],3])

        self.before_obstacle = [def_ for def_ in self.before_obstacle if def_[2] > 0]

        obj_collector = [def_ for def_ in self.before_obstacle]
        ...
        for bbox_center_x, bbox_center_y,timer in obj_collector:
            if(self.State=="Rubber_cone_drive"):
                ...
                '''if rfirst == False and 0 < bbox_center_x < 3  and -3 < bbox_center_y < 0.1:
                    print("R find")
                    rfirst = True
                    right_cones.append(new_cone)
                elif rfirst == True and 1 > bbox_center_y >-1.5:
                    if self.should_add_cone(right_cones, new_cone, self.min_distance_threshold ):
                        right_cones.append(new_cone)'''
            #print(bbox_center_x,bbox_center_y,bbox_width,bbox_height)
        #print("!!!!!!!!!!!!!!!!!!!!!!!!")
        #print(len(self.left_cones),len(self.right_cones))
        if self.find_OK_bet_R_L_bool(left_cones,right_cones):
            if left_cones[1][0]<right_cones[1][0]:
                left_cones.extend(right_cones[1:])
                right_cones = [right_cones[0]]
            else:
                right_cones.extend(left_cones[1:])
                left_cones = [left_cones[0]]
        ...
        if len(left_cones) >= 2 and len(right_cones) < 2:
            ...
            #left_curve = self.generate_spline(self.left_cones,self.line_space)
            ...
            #right_curve = self.generate_spline(self.right_cones,self.line_space)
