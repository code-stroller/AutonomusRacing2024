...
class GPS2UTM:
    def __init__(self):
        rospy.loginfo("parking distance finding")
        self.State=""
        self.start_lid=False
        # ------------------------- Subscriber ----------------------
        ...
        rospy.Subscriber("/start_lidar", Bool, self.start_lid_callback)
        rospy.Subscriber("/lidar/tracking_objects", TrackingObjectArray, self.lidar_callback)
        ...
        self.middle_point_pub = rospy.Publisher('raytracking', MarkerArray, queue_size=10)
        # ------------------------- Publisher ----------------------------
    ...
    def start_lid_callback(self,data):
        self.start_lid=data.data  
    ...
    def lidar_callback(self, _data):
        if not self.start_lid or self.State!="Parking_path":
            return
        ...
        if j != -1:
            if len(final)==2:
                a = np.sqrt(final[1][0]**2 + final[1][1]**2)
                b = np.sqrt(final[0][0]**2 + final[0][1]**2)
...
def run():
    rospy.init_node("parkingdistance")
    new_classs= GPS2UTM()
    rospy.spin()
