...
class GPS2UTM:
    def __init__(self):
        ...
        self.lfirst=False
        self.rfirst=False
        self.min_distance_threshold = 3.1
    ...
    def interpolate_and_fill(self, obs_1, obs_2, occ_grid_map, max_len, max_wid, origin,fill_probs,yaw_rad):
        ...
        if abs(obj_x1 - obj_x2) >= abs(obj_y1 - obj_y2):
            for j in np.arange(obj_x1, obj_x2, self.resolution):
                ...
                self.fill_map(occ_grid_map, j, y, max_len, max_wid, self.resolution, self.look_up_L, origin,fill_probs,yaw_rad)
        else:
            for j in np.arange(obj_y1, obj_y2, self.resolution):
                x = (j - obj_y1) * (obj_x1 - obj_x2) / (obj_y1 - obj_y2) + obj_x1
                self.fill_map(occ_grid_map, x, j, max_len, max_wid, self.resolution, self.look_up_L, origin,fill_probs,yaw_rad)
    ...
    def lidar_callback(self, _data):
        ...
        paaath_dis = []
        for j in range(1,len(distance_list2)-1):
            paaath_dis.append(distance_list2[j-1]+distance_list2[j+1]+distance_list2[j])
