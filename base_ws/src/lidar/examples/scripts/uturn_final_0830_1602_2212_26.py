    ...
    def interpolate_and_fill(self, obs_1, obs_2, occ_grid_map, max_len, max_wid, origin,fill_probs,yaw_rad):
        ...
        if abs(obj_x1 - obj_x2) >= abs(obj_y1 - obj_y2):
            for j in np.arange(obj_x1, obj_x2, self.resolution):
                ...
                self.fill_map(occ_grid_map, j, y, self.resolution*7, self.resolution*3, self.resolution, self.look_up_L, origin,fill_probs,yaw_rad)
        else:
            for j in np.arange(obj_y1, obj_y2, self.resolution):
                x = (j - obj_y1) * (obj_x1 - obj_x2) / (obj_y1 - obj_y2) + obj_x1
                self.fill_map(occ_grid_map, x, j, self.resolution*7, self.resolution*3, self.resolution, self.look_up_L, origin,fill_probs,yaw_rad)
    ...
    def lidar_callback(self, _data):
        ...
        for obs_1, obs_2 in pairs:
            ...
            if obs_1 == obs_2:
                ...
                self.interpolate_and_fill(obs_1, obs_2, occ_grid_map, max_len, max_wid, origin,1,obj_yaw1)
        ...
        if np.mean(paaath_dis) >=400:
            uturn_cone = path[int(self.num_path_point) +3]
            #print(int(self.num_path_point/2))
        else:
            #abc = np.clip(paaath_dis[paaath_dis.index(min(paaath_dis)):].index(max(paaath_dis[paaath_dis.index(min(paaath_dis)):]))+2+paaath_dis.index(min(paaath_dis)),0,len(path))
            #print(abc)
            abc = np.clip(paaath_dis.index(max(paaath_dis)),0,len(path))
            uturn_cone = path[abc]
    ...
    print(ans)
        ...
        if(uturn_cone != [] ):
            ...
            if len(uturn_cone)<5:
                ...
                target_point.data.append(1)
                target_point.data.append(uturn_cone[5][1]/uturn_cone[5][0]*3)
