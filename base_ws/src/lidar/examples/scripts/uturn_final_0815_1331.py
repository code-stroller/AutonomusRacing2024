...
class GPS2UTM:
    def __init__(self):
        ...
        self.lfirst=False
        self.rfirst=False
        self.min_distance_threshold = 2.4  
    ...
    def lidar_callback(self, _data):
        ...
        # 2차원 배열로 변환하여 데이터를 출력하거나 처리
        matrix = []
        for i in range(rows):
            row = _data.data[i * cols:(i + 1) * cols]  # 각 행의 데이터를 슬라이싱
            matrix.append(row)
        ...
        '''
        obj_collector = matrix
        left_cones = []
        right_cones = []
        print((obj_collector))
        left_cones .append((-0.4, 3))
        right_cones.append((-0.4, -3))
        obj_collector.sort(key=lambda x: x[0])
        lfirst = False
        rfirst = False

        #for bbox_center_x, bbox_center_y,bbox_width, bbox_height in obj_collector:
        for bbox_center_x, bbox_center_y in obj_collector:
            if(self.State=="Rubber_cone_drive"):
                ...
                elif lfirst == True and -1.5 < bbox_center_y < 0.8:
