...
I_GAIN_2 = 0.45
D_GAIN_2 = 0
SECOND_VELOCITY_KPH = 10



# first_velocity
FIRST_VELOCITY_KPH = 5 #추가
error_rate = 0.2
...
class Longitudinal_control :
    ...
    def main(self):
        ...
        target_velocity = int(round(self.pid_2.pid(target_vel = FIRST_VELOCITY_KPH*10, current_vel = curr_velocity)))
        
        if self.is_first:
            target_velocity = int(round(self.pid.pid(target_vel = SECOND_VELOCITY_KPH * 10, current_vel = curr_velocity)))
            ...
            # 데이터가 60개 모이면 2초
            if len(self.velocity_array) == 120:
                self.is_first = True 
                return
