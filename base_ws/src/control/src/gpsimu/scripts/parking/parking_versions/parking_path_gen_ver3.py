...
class Oblique_parking:
    ...
    def __init__(self):
        ...
        self.heading_adjust_trigger = rospy.Publisher("/heading_adjust_trigger", Bool, queue_size=1)
    ...
    def handle_heading_adjust(self):
        print("adjusting heading" + str(self.diff_heading(self.vehicle_yaw, self.park_heading_yaw)))
        bool_msg = Bool()
        bool_msg.data = False
        self.heading_adjust_trigger.publish(bool_msg)

        self.erp_msg.gear = 0
        self.erp_msg.steer = 2000
        self.erp_msg.speed = 2
        self.erp_msg.brake = 0
        self.erp_42_ctrl_pub.publish(self.erp_msg)

        if self.diff_heading(self.vehicle_yaw, self.park_heading_yaw) < math.radians(5):
            print("adjust heading complete")
            self.erp_msg.gear = 0
            self.erp_msg.steer = 0
            self.erp_msg.speed = 0
            self.erp_msg.brake = 20
            self.erp_42_ctrl_pub.publish(self.erp_msg)
            time.sleep(2)
            self.erp_msg.gear = 0
            self.erp_msg.steer = 0
    ...
    def diff_heading(self, car_heading, park_heading):
        diff_heading_before = car_heading - park_heading
        if diff_heading_before > np.pi:
            diff_heading_before -= 2*np.pi
        elif diff_heading_before < -np.pi:
            diff_heading_before += 2*np.pi
        
        return abs(diff_heading_before)
