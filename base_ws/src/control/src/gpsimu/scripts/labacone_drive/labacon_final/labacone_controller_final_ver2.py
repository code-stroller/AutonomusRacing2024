...
class LabaconDrive:
    ...
    def control_labacone_velocity(self, steering):
        steering_angle_deg = abs(steering) / 2000 * 28.2
        ...
        sigmoid = 1 / (1 + math.exp(k * (steering_angle - threshold)))
        target_velocity = min_speed + (max_speed - min_speed) * sigmoid

        output = round(self.pid.pid(target_velocity, self.erpStatus_msg.speed))
