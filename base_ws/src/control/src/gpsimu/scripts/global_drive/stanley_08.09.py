...
# ==============================================================================================================================================================
DESIRED_VELOCITY_KPH_FOR_GLOBAL = 15
DESIRED_VELOCITY_KPH_FOR_INTERSECTION = 15
steer_vel_for_global = 8
steer_vel_for_intersection = 10
...
add_name = 'stanley_intersection_pid_i_0.2_d_0.002'
...
# ==========================================================================================
    ...
    def calc_stanley(self):
        ...
        if curr_velocity_kmh != 0:
            steering = pi_t + np.arctan(k_s * x_t / curr_velocity_kmh)
    ...
    def cal_control(self):
        ...
        if self.Path_state == "Global_path":
            if (abs(steering)<math.radians(5.)):
                target_velocity = DESIRED_VELOCITY_KPH_FOR_GLOBAL * 10
            elif (abs(steering)<0.4922):
                target_velocity = steer_vel_for_global * 10 # erpunit
            else:
                target_velocity = steer_vel_for_global * 10 # erpunit

        elif self.Path_state == "Intersection_path":
            if (abs(steering)<math.radians(5.)):
                target_velocity = DESIRED_VELOCITY_KPH_FOR_INTERSECTION * 10
            elif (abs(steering)<0.4922):
                target_velocity = steer_vel_for_intersection * 10 # erpunit
            else:
                target_velocity = steer_vel_for_intersection * 10 # erpunit
        ...
        if curr_velocity > target_velocity+20:
            target_velocity = 0
            brake = 20
        elif DESIRED_VELOCITY_KPH_FOR_GLOBAL == 0:
            brake = 200
        elif DESIRED_VELOCITY_KPH_FOR_INTERSECTION == 0:
            brake = 200
        else:
            brake = 0
