...
class State_machine:
    ...
    def __init__(self):
        ...
        # rospy.Subscriber("/observed_sign", Bool, self.observed_callback) # 배달 미션에서 언제 멈출것인가(담당자: 유지상님 & 방지윤님)
        # rospy.Subscriber("/stopLine", Float32MultiArray, self.stop_callback)# 정지선 유무: 0. -> False, 1. -> True, y픽셀 : float (담당자: 신규민님)
        #------------------------------------------------------------------------------

        #--------------------------------Publisher--------------------------------------
        self.Desired_velocity_publisher = rospy.Publisher('/desired_velocity', Int32, queue_size=1) # 원하는 속도를 제어기에 넘기기(모든 코드에서 속도는 target_velocity로 바꾸기)
        # self.utm_cal_trigger_pub = rospy.Publisher("utm_cal_trigger", Bool, queue_size = 1) # 배달 좌표 계산하게 트리거 발행
    ...
    def State_space(self):
        ...
        #         if self.cal_error(self, self.x, self.y, self.pick_x, self.pick_y) < 0.3: # 목표 지점 n m이내로 들어오면
        #             self.stop() # 정지 명령 보내기
    ...
    def drive(self):
        ...
        if (351 < self.index and self.index < 376) or (440 < self.index and self.index < 467) or (670 < self.index and self.index < 703) or (855 < self.index and self.index < 884) or (997 < self.index and self.index < 1019) or (1503 < self.index and self.index < 1529) or (1581 < self.index and self.index < 1603):
    ...
    def intersection_path_pub(self):

        self.Status_msg = "Intersection Path Drive"
        self.Path_state = "Intersection_path"
    ...
    #         self.stop() # 일단 멈추고
    #         self.utm_cal_trigger_pub.publish(True) # 계산하라고 방지윤님께 보내기
    #     else:
    #         self.utm_cal_trigger_pub.publish(False)
    ...
    def stop(self): # 정지선이나 정지할 때, 목표속도를 0으로 하는 함수
        ...
        int_msg = Int32()
        int_msg.data = 0
        self.Desired_velocity_publisher.publish(int_msg) 
