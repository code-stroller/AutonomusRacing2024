...
degree_mod = 30
degree_to_rad = math.radians(degree_mod)
...
# ==========================================================================================
    ...
    def calc_pure_pursuit(self):    # pure_pursuit 계산 값을 이용하며 경로 생성
        ...
        if self.vehicle_yaw + degree_to_rad>np.pi:
            left_sight_rad = self.vehicle_yaw + degree_to_rad - 2*np.pi
        else:
            left_sight_rad = self.vehicle_yaw + degree_to_rad

        if self.vehicle_yaw - degree_to_rad < -np.pi:
            right_sight_rad = self.vehicle_yaw - degree_to_rad + 2*np.pi
        else:
            right_sight_rad = self.vehicle_yaw - degree_to_rad


        # 삼각함수 계산에는 라디안이 사용됨. vehicle_length 도 실제 값
        Lp_x_2 = self.current_position.x + Lp * np.cos(left_sight_rad)
        Lp_y_2 = self.current_position.y + Lp * np.sin(left_sight_rad)
        Lp_x_3 = self.current_position.x + Lp * np.cos(right_sight_rad)
        Lp_y_3 = self.current_position.y + Lp * np.sin(right_sight_rad)

        dis_P2_2 = np.sqrt((np.array(ref_x) - Lp_x_2)**2+(np.array(ref_y) - Lp_y_2)**2)
        dis_P2_3 = np.sqrt((np.array(ref_x) - Lp_x_3)**2+(np.array(ref_y) - Lp_y_3)**2)
    

        min_index_2 = np.argmin(dis_P2_2)
        min_index_3 = np.argmin(dis_P2_3)

        min_values = [dis_P2_2[min_index_2], dis_P2_3[min_index_3]]
        overall_min_value = min(min_values)
        overall_min_index = min_values.index(overall_min_value)

        if overall_min_index == 0:
            final_array = "dis_P2_2"
            final_index = min_index_2
            Way_x = ref_x[min_index_2]
            Way_y = ref_y[min_index_2]
            Lp_x = Lp_x_2
            Lp_y = Lp_y_2
        else:
            final_array = "dis_P2_3"
            final_index = min_index_3
            Way_x = ref_x[min_index_3]
            Way_y = ref_y[min_index_3]
            Lp_x = Lp_x_3
            Lp_y = Lp_y_3
        ...
        self.visualization_heading_WP([Lp_x, Lp_y],[Way_x, Way_y], self.current_position,final_index)
        print("=================================")
        if steering>0:
